...
@Injectable()
export class UsersCompanySupervisorsService {

  private defaultLimit: number;
  private role: string = 'companySupervisor'
  ...
  public findUsers = async (paginationDto: any = {}, companyId: string) => {
    ...
    const databaseRole = await this.roleModel.findOne({ name: 'companySupervisor' as string })
    if(!databaseRole) {
      throw new NotFoundException(`Role with name 'companySupervisor' not found`)
    }
      ...
      if(userExist) {
        ...
        if(routesArray && routesArray.length) {
          ...
          for (let index = 0; index < routesArray.length; index++) {
            const routeId = routesArray[index];
            const newRouteResponse = await this.routeModel.findById(routeId).populate('users')
            if (!newRouteResponse) {
              throw new NotFoundException(error.ROUTE_NOT_FOUND);
            }

            // Borra la ruta del usuario anterior
            const havePreviousUser = newRouteResponse.users.find((routeUser) => routeUser.role === this.role)
            if (havePreviousUser !== undefined) {
              const previousUser = await this.userModel.findById(havePreviousUser.user).populate('routes')
              if(!previousUser) {
                throw new NotFoundException(error.USER_NOT_FOUND)
              }
              previousUser.routes = previousUser.routes.filter((routeUser) => String(routeUser.route) === String(routeId))
            ...
            newRouteResponse.users = newRouteResponse.users.filter((routeUser) => routeUser.role !== this.role)
            newRouteResponse.users.push(createdRouteUser.id)
            newRouteResponse.updatedAt = currentDateTime
  
            await newRouteResponse.save()
          }
        }
  ...
  public update = async (id: string, updateUserDto: UpdateUserDto, userRequest: User, clientIp: string): Promise<object> => {
    try {
      ...
      // Se deben eliminar todas las rutas actuales
      // Se debe eliminar el supervisor de las rutas en que está
      userResponse.routes = []

      for (let index = 0; index < routes.length; index++) {
        const routeUser = routes[index];
        const route = await this.routeModel.findById(routeUser.route).populate('users')
        if (!route) {
          throw new NotFoundException(error.ROUTE_NOT_FOUND);
        }

        route.users = route.users.filter((routeUser) => routeUser.role !== this.role)
        await Promise.all([
          routeUser.deleteOne(),
          route.save(),
        ])
