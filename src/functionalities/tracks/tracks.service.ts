...
import { TrackReturnData } from './interfaces/track-return-data.interface'
...
export class TracksService {
  ...
  /**
   * Formats the return data for a track and their associated companies. This includes structuring track data and
   * compiling a list of associated companies.
   *
   * @private
   * @function formatReturnData
   * @param {Track} track - The track object containing track details.
   * @returns {TrackReturnData} An object containing the formatted track and company data.
   */
  private formatReturnData = (track: Track): TrackReturnData => {
    const { user } = track
    return {
      ip: track?.ip || '',
      ...
      user: user ? `${ this.utils.capitalizeFirstLetter(user.firstName).trim() } ${ this.utils.capitalizeFirstLetter(user.paternalSurname).trim() }` : 'System',
    }
  }
    ...
    try {
      ...
      return {
        data: {
          ...
          tracks: tracks?.docs.map((el) => this.formatReturnData(el)),
        }
      }
    } catch (error) {
